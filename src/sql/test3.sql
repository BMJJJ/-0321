...
-- 기본키(primary key) : 테이블을 대표하는 키, 중복을 허용하지 않는다. 기본키는 여려개가 올 수 있다.
create table test1(
...
insert into test2 values (1,'소나무',55,'ccc'); --- 1.o, 2.x
insert into test2 values (default,'소나무',55,'bbb'); --1.o 2.o
select * from test2;

--UNIQUE KEY 중복불허를 위해 설정하는키(PRIMARY KEY를 대신해서 중복을 불허시키고자할때 사용한다.)
create table test3(
...
/* 외래키(Foreign key)
   하나의 테이블에서 다른테이블의 정보를 찾기위해서 연결해주는 역활을 할때 지정하는 키이다.
   조건, 현재 테이블의 필드에 외래키로 설정하려한다면? 반드시 상대쪽 테이블의 해당필드는 primary key이거나 unique key로 등록되어 있어야 한다.
   또한, 외래키로 지정하려는 필든느 상대쪽 테이블의 해당필드속성과 같아야 한다.
 */
...
select t3.idx as 고유번호,t4.gender 성별 from test3 t3, test4 t4;

select t3.*,t4.gender from test3 t3,test4 t4 where t3.test3Code = t4.test4Code;
