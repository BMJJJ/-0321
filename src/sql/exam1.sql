...
create table hoewon(
	...
	ipsail datetime not null default now(), 
	address varchar(50)
);
...
alter table sinsang change column content sogo text;

-- 27. sinsang테이블의 soge필드의 타입을 가변길이 100Byte로 변경하시오.(null값은 허용한다)

alter table sinsang change column content sogo varchar(100);

-- 28. sinsang테이블에 content 필드를 삭제하시오.(실제로 삭제하지는 말고 명령어만 사용하시오. 만약 삭제하였다면 새롭게 content필드를 가변길이 100Byte로 추가하시오)

alter table sinsang drop column contant;

-- 29. sinsang테이블 구조보기

desc sinsang;

-- 30. sinsang테이블의 gender필드명을 sex필드명으로 변경하시오.(고정길이문자(2), 기본값(여자))

alter table sinsang change column gender sex char(2) default(여자);
...
select * from sinsang where  address not in('인천', '서울') and gender = '여자';
update sinsang set point = point + 500 where  address not in('인천', '서울') and gender = '여자';

-- 54. 1990년 이전에 입사한 남자회원들에 대하여는 모든 포인트를 회수하시오.
select * from sinsang where gender ='남자' and ipsail <'1990-01-01';
